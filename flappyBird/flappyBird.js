...
function Play()
{
    ...
    var pipe_seperation = 0; // when pipes will appear
    var pipe_gap = 35;
    ...
    function create_pipe (){
        ...
        if (pipe_seperation > 115){
            pipe_seperation = 0;
            var pipe_posi = Math.floor(Math.random() * 43) +8; //0.4 * 43 = floor(17.2) = 17 + 8 = 25 
            var pipe_sprite_inv = document.createElement('div'); //create top pipe (inverse)
            pipe_sprite_inv.className = 'pipe_sprite';
            pipe_sprite_inv.style.top = pipe_posi - 70 + 'vh'; // 25 - 70 = -45 (start draw at -45)
            pipe_sprite_inv.style.left = '100vw';

            $(document.body).append(pipe_sprite_inv);

            var pipe_sprite = document.createElement('div'); //create bottom pipe 
            pipe_sprite.className = 'pipe_sprite';
            pipe_sprite.style.top = pipe_posi + pipe_gap + 'vh'; // 25 + 35 = 60 (start draw at 60)
